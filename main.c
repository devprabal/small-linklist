...
{
    Config config = {.list_data_alloc_func_fptr = my_list_data_calloc_func,
    ...
    destroy_list(head, NULL, NULL); ////TODO: How will user know that once he has set the dealloc func, does he also need to pass it here or not? or does he need to pass something else like `user_data_free_func()`?
}
...
void out_of_mem_tc(void)
{
    ...
    assert(SLL_SUCCESS == append_to_list(head, sizeof(Person), &(Person){.name = strdup("Deleted Account1"), .age = 25}));
...
int main(void)
{
    ...
    //// Test out this case and discuss if it is the correct behaviour, or should be stop making Nodes if Node->data filling fails?
    //// In that case, we will also need to delete the created Node for which data filling fails
    return 0;
}
